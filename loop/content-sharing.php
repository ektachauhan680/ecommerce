...
<div class="share <?php if( get_post_type() !== 'post' ) echo 'text-center'; ?>"> 
    ...
    <a href="http://www.reddit.com/submit?url=<?php the_permalink(); ?>" onClick="return ebor_reddit()" target="_blank" class="btn share-reddit">Reddit</a>
    <a href="http://www.linkedin.com/shareArticle?mini=true&url=<?php the_permalink(); ?>" onClick="return ebor_linkedin()" target="_blank" class="btn share-facebook">LinkedIn</a>
</div>
...
<script type="text/javascript">
	...
	function ebor_reddit() {
		window.open('http://www.reddit.com/submit?url=<?php the_permalink(); ?>&media=<?php echo $url[0]; ?>&description=<?php echo sanitize_title(get_the_title()); ?>','sharer','toolbar=0,status=0,width=626,height=436');
		return false;
	}
	function ebor_linkedin() {
		window.open('http://www.linkedin.com/shareArticle?mini=true&url=<?php the_permalink(); ?>&media=<?php echo $url[0]; ?>&description=<?php echo sanitize_title(get_the_title()); ?>','sharer','toolbar=0,status=0,width=626,height=436');
		return false;
	}
</script>