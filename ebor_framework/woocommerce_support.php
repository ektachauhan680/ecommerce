	...
	<?php 				
		foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item) :
			...
			if ($_product->exists() && $cart_item['quantity']>0) :
				...
				echo '<span class="quantity">' .$cart_item['quantity'].' &times; '.wc_price($_product->get_price()).'</span>
		...
		foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item) :
			...
			if ($_product->exists() && $cart_item['quantity']>0) :
				...
				if (has_post_thumbnail($cart_item['product_id'])) :					
					...
					$output .=  '<img src="'.$woocommerce->plugin_url(). '/assets/images/placeholder.png" alt="Placeholder" width="'.$woocommerce->wc_get_image_size('shop_thumbnail_image_width').'" height="'.$woocommerce->wc_get_image_size('shop_thumbnail_image_height').'" />'; 				
				endif;				
				$output .=  '</a>';
				$output .=  '</div>';
				...
				$output .=  '<span class="quantity">' .$cart_item['quantity'].' &times; '.wc_price($_product->get_price()).'</span>
					  </a></div><div class="clear"></div></li>';
